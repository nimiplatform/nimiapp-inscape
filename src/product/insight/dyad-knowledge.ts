...
const DOMINANT_RELATION_DYNAMICS_BY_LOCALE: Record<
  InscapeLocale,
...
> = {
  zh: {
    ...
    same_axis_opposite_attitude: {
      ...
      friction:
        '方向相反——一个向内一个向外；常因「闭合/推进 vs 开放/探索」的节奏互相觉得对方急或慢。',
      bridge:
        '先点破「我们要的其实是同一个东西」，再就节奏定一个共同约定（例如「探索到某点再拍板」）。',
    },
    different_axis: {
      resonance: '不同轴：你们可能先注意到不同的部分，有机会在视角上互补。',
  ...
  en: {
    same_function: {
      resonance:
        'Same dominant function: you use the same core way of understanding the world, so being understood can arrive quickly.',
      friction:
        'Similarity can become competition on the same track, while both of you miss the blind spot you share.',
      bridge:
        'Divide roles deliberately and bring in a third perspective for what neither of you naturally sees.',
    },
    same_axis_opposite_attitude: {
      resonance:
        'Same axis, opposite attitude: you care about the same underlying thing, so the abstract resonance is real.',
      friction:
        'The direction differs, inward versus outward; pacing conflicts often show up as one side feeling rushed and the other feeling stalled.',
      bridge:
        'Name that you are protecting the same underlying concern, then agree on rhythm, such as exploring to a threshold before deciding.',
    },
    different_axis: {
      resonance:
        'Different axis: you may notice different parts of the same situation, offering potentially complementary perspectives.',
      friction:
        'Rapport needs translation; what one side values may not be automatically visible to the other.',
      bridge:
        'Translate your perspective into the other persons language: experience, results, or possibilities. Do not assume they automatically get it.',
    },
  },
};
...
export const AXIS_OPPOSITE_DYNAMICS: Record<string, string> = AXIS_OPPOSITE_DYNAMICS_BY_LOCALE.zh;
